...
{
  // create queue for sensor-server task communication
  vQueueDelete(server_sensor_queue);
...
void serverCommunicationTask(void *parameter)
{
  ...
  static int16_t hour_old, hour_new;

  // set variables
  SDU_getDay(&day_old);
  SDU_getDay(&day_new);

  SDU_getHour(&hour_old);
  SDU_getHour(&hour_new);
  ...
  for(;;)
  {
    ...
    //{
      uint8_t ret = FOTA_checkNewVersion((*(serverCommunicationTaskParams_s *)parameter).sdu_s);
      if (ret == FOTA_OK)
      {
        Serial.println("New version of firmware detected");
